package indi.ayun.mingwork_all.swipeitem.recyclerview;
...
public class OldSwipeItemLayout extends ViewGroup {
    enum Mode{
    ...
    public OldSwipeItemLayout(Context context) {
        this(context,null);
    }

    public OldSwipeItemLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    class ScrollRunnable implements Runnable{
        ...
        void startScroll(int startX,int endX){
            if(startX!=endX){
                ...
                ViewCompat.postOnAnimation(OldSwipeItemLayout.this,this);
            }
        }
    ...
    public static class OnSwipeItemTouchListener implements RecyclerView.OnItemTouchListener {
        private OldSwipeItemLayout mCaptureItem;
        ...
        public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent ev) {
            ...
            switch (action){
                case MotionEvent.ACTION_DOWN:{
                    ...
                    OldSwipeItemLayout pointItem = null;
                    //首先知道ev针对的是哪个item
                    View pointView = findTopChildUnder(rv,(int)x,(int)y);
                    if(pointView==null || !(pointView instanceof OldSwipeItemLayout)){
                        //可能是head view或bottom view
                        pointOther = true;
                    }else
                        pointItem = (OldSwipeItemLayout) pointView;
    ...
    public static void closeAllItems(RecyclerView recyclerView){
        for(int i=0;i<recyclerView.getChildCount();i++){
            ...
            if(child instanceof NewSwipeItemLayout){
                OldSwipeItemLayout oldSwipeItemLayout = (OldSwipeItemLayout) child;
                if(oldSwipeItemLayout.isOpen())
                    oldSwipeItemLayout.close();
            }
        }
    }
