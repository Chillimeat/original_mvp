...
import indi.ayun.original_mvp.base.BaseFragment;
import indi.ayun.original_mvp.media_box.model.BoxingBuilderConfig;
...
import indi.ayun.original_mvp.media_box.presenter.PickerContract;
import indi.ayun.original_mvp.media_box.utils.CameraPickerHelper;
...
public abstract class AbsBoxingViewFragment extends BaseFragment implements PickerContract.View {
    ...
    private void checkPermissionAndLoad() {
        try {
            if (!BoxingBuilderConfig.TESTING &&
                    createPermission(STORAGE_PERMISSIONS[0],STORAGE_PERMISSIONS[1]).checkNo()) {
    ...
    public final void startCrop(@NonNull BaseMedia media, int requestCode) {
        ...
        BoxingCrop.getInstance().onStartCrop(activityContext(), this, cropConfig, media.getPath(), requestCode);
    }
     ...
     * @param subFolderPath the folder name in "DCIM/ClockLiving/boxing/"
     */
    public final void startCamera(Activity activity, Fragment fragment, String subFolderPath) {
        try {
            if (!BoxingBuilderConfig.TESTING && createPermission(CAMERA_PERMISSIONS[0]) .checkNo()) {
