...
public class ResourceUtil extends UtilBase {
	...
	public static List<String> readResList(String type,Object value){
		...
		switch (type){
			case RAW://根据本地Raw目录下资源标识，获取List集合信息
				...
				try {
					InputStreamReader in = new InputStreamReader(OriginalMVP.getContext().getResources().openRawResource(resId));
			...
			case ASSETS://根据本地Assets目录下资源名称，获取List集合信息
				...
				try {
					InputStreamReader in = new InputStreamReader(OriginalMVP.getContext().getResources().getAssets().open(fileName));
	...
	public static byte[] readResBytes(String type,Object value){
		...
		switch (type){
			case RAW://根据本地Raw目录下资源标识，获取List集合信息
				...
				try {
					is = OriginalMVP.getContext().getResources().openRawResource(rawId);
			...
			case ASSETS://根据本地Assets目录下资源名称，获取List集合信息
				String fileName=value.toString();
				try {
					is = OriginalMVP.getContext().getAssets().open(fileName);
					int size = is.available();
					buffer = new byte[size];
					is.read(buffer);
				} catch (IOException e) {
	...
	public static String getStringById(int strId){
		return OriginalMVP.getContext().getResources().getString(strId);
	}
	...
	public static int getColorById(int colorId){
		return OriginalMVP.getContext().getResources().getColor(colorId);
	}
	...
	public static Drawable getDrawableById(int drawableId){
		return OriginalMVP.getContext().getResources().getDrawable(drawableId);
	}
