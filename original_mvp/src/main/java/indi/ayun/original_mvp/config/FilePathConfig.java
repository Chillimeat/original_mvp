...
import static indi.ayun.original_mvp.utils.storage.FileUtils.isMountedSDCard;
import static indi.ayun.original_mvp.utils.storage.FileUtils.isRootPath;
...
public class FilePathConfig {
    ...
    public static String BUGFILE_PATH = Environment.getExternalStorageDirectory()
            + File.separator + OriginalMVP.getContext().getString(R.string.app_name)
    ...
    public static String ENV_ROOT = Environment.getExternalStorageDirectory()
            + File.separator
            + OriginalMVP.getContext().getString(R.string.app_name)
            + File.separator;
    ...
    public static String CON_CACHE = OriginalMVP.getContext().getExternalCacheDir().getPath();

    //当你用户卸载时，不会留下来作为垃圾文件。/Android/data/包名/files目录
    public static String CON_FIle = OriginalMVP.getContext().getExternalFilesDir(null).getPath();
