...
public class CredentialPreferences extends UtilBase {
    ...
    static SharedPreferences getSharedPreferences() {
        return OriginalMVP.getContext().getSharedPreferences(OriginalMVP.getAppName(), Context.MODE_PRIVATE);
    }
    ...
    public static boolean checkFirst() {
        if (IsNothing.onDataStr(CredentialPreferences.getAndroidId())) {
            CredentialPreferences.saveAndroidId(Info.getAndroidID(OriginalMVP.getContext()));
            return true;
        } else {
            return false;
        }
