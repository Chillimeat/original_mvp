...
/**
 * SharedPreferenceProvider里会访问真正的SharedPreference
 * 重要差别的地方在这里：在调用getSharedPreferences时，会先判断caller的进程pid是否与SharedPreferenceProvider相同。如果不同，则返回SharedPreferenceProxy。如果相同，
 * 则返回ctx.getSharedPreferences。只会在第一次调用时进行判断，结果会保存起来。
 * 只有当调用者是正真跨进程时才走的contentProvider
 *
 * 在跨进程的SharedPreferenceProxy 里，registerOnSharedPreferenceChangeListener暂时还没有实现，可以使用ContentObserver实现跨进程监听。
 */
public class SharedPreferenceProvider extends ContentProvider {
    ...
    /**
     * 所有跨进程的操作都是通过SharedPreferenceProvider的call方法完成。
     * @param method
     * @param arg
     * @param extras
     * @return
    ...
    private MethodProcess methodQueryPid = new MethodProcess() {
        ...
        public Bundle process(@Nullable String arg, @Nullable Bundle extras) {
            ...
            bundle.putInt(PreferenceUtil.KEY_VALUES, android.os.Process.myPid());
            return bundle;
        }
    };
